...
describe("「使い方」ページの中身", () => {
  ...
  it("★各画面の目印（slug）が一意で、モーダルのタブに使える形になっている", () => {
    const slugs = HELP_SECTIONS.map((s) => s.slug);
    expect(new Set(slugs).size).toBe(slugs.length);
    for (const slug of slugs) expect(slug).toMatch(/^[a-z]+$/);
  });

  it("★「使い方」はモーダルで出す（ページではない）。写真を出す部品を読み込んでいる", () => {
    // 2026-09-22: /help・/help/<slug> のページから、どこからでも開けるモーダルに変えた
    expect(existsSync("app/help")).toBe(false);
    const dialog = readFileSync("components/help-dialog.tsx", "utf8");
    expect(dialog).toContain("HelpShots");
    expect(dialog).toMatch(/from\s+["']@\/components\/help-shots["']/);
  });
