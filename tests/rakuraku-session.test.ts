...
describe("ログイン状態の封印", () => {
  it("封じて開くと元に戻る", () => {
    expect(unseal(seal(STATE)).state).toBe(STATE);
  });
  ...
  it("毎回違う値になる (同じ入力でも使い回せない)", () => {
    expect(seal(STATE)).not.toBe(seal(STATE));
  });
