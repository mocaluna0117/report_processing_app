// タブを移動しても顛末書の接続が切れないようにするための、このページ読み込み限りの控え。
//
// 画面 (/ ・/after ・/tenmatsu) はそれぞれ別のルートなので、タブを切り替えると
// 顛末書の画面は unmount され、接続の状態が初期値に戻る。そのたびに「接続する」を
...
export const tenmatsuSession: TenmatsuSession = {
  hydrated: false,
  token: null,
  editingToken: false,
  connection: "idle",
...
export function keepTenmatsuSession(next: Omit<TenmatsuSession, "hydrated">): void {
  Object.assign(tenmatsuSession, next, { hydrated: true });
}

/** テスト用。控えを初期状態に戻す */
export function resetTenmatsuSession(): void {
  Object.assign(tenmatsuSession, {
    hydrated: false,
    token: null,
    editingToken: false,
