 ...
 * ★消せない約束（パスワードを保存しない・PDFは folio のサーバーに残さない）は、畳んでも必ず残す。
 */
export function saveNote(kind: DocKind): SafetyNote {
  return {
    summary: "保存するのはこのブラウザの中だけです (楽楽精算のパスワードは保存しません)。",
    details: [
      `このブラウザに保存するもの: 取得済み一覧の写し (伝票№・${kind.text.sensitiveFields}・${kind.text.flagMarks}。物件名には施主名が入ることがあります)、保存先フォルダーの場所、楽楽精算のログインID、1回に取る件数、選んだ部門。`,
      `${kind.label}のPDFは楽楽精算から folio のサーバーを通ってこのブラウザに届き、選んだフォルダーにだけ保存されます (folio のサーバーには残しません)。`,
      "楽楽精算のパスワードはログインに使うだけで、どこにも保存しません (このブラウザのメモリにだけ置き、読み込み直すと消えます)。ログイン状態は暗号化してこのタブにだけ残り、タブを閉じると消えます (期限は8時間)。",
