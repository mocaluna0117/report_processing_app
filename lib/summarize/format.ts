...
export interface FormatOptions {
  ...
  emptyText?: string;
  /**
   * 事象ごとの補足 (items と同じ並び。空文字なら補足なし)。
   * 事象の次の行に「補足: …」として入れる (完了報告書の指示内容の下に書く1行)。
   */
...
export function formatPhenomena(
  ...
  const clean = items
    .map((s, i) => ({
      ...
      supplement: (options.supplements?.[i] ?? "").trim(),
    }))
  ...
  const withSupplement = (head: string, supplement: string) =>
    supplement ? [head, `${SUPPLEMENT_PREFIX}${supplement}`] : [head];
  const body =
    clean.length === 0
      ? emptyText
        ? [emptyText]
        : []
      : clean.length === 1
        ? withSupplement(clean[0].text, clean[0].supplement)
        : clean.flatMap((pair, i) =>
            withSupplement(`${circledNumber(i + 1)}${pair.text}`, pair.supplement),
          );
